...
exports.postsRouter.get('/feed', auth_middleware_1.authenticateToken, async (req, res) => {
    try {
...
// Create Post
exports.postsRouter.post('/', auth_middleware_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
...
// Like / Unlike Post
exports.postsRouter.post('/:id/like', auth_middleware_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        ...
        if (existing) {
            ...
            const [, post] = await prisma_1.prisma.$transaction([
                prisma_1.prisma.like.create({ data: { userId, postId } }),
                prisma_1.prisma.post.update({
                    where: { id: postId },
...
// Save / Bookmark Post
exports.postsRouter.post('/:id/save', auth_middleware_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        const postId = req.params.id;
        const existing = await prisma_1.prisma.savedPost.findUnique({
            where: { userId_postId: { userId, postId } },
        });
        if (existing) {
            await prisma_1.prisma.savedPost.delete({ where: { id: existing.id } });
            return res.json({ saved: false });
        }
        else {
            await prisma_1.prisma.savedPost.create({ data: { userId, postId } });
...
// Comments for a post
exports.postsRouter.get('/:id/comments', auth_middleware_1.authenticateToken, async (req, res) => {
    try {
        const postId = req.params.id;
...
// Add comment
exports.postsRouter.post('/:id/comments', auth_middleware_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        const postId = req.params.id;
        ...
        const [comment, post] = await prisma_1.prisma.$transaction([
            prisma_1.prisma.comment.create({
                data: {
                    postId,
                    authorId: userId,
...
exports.postsRouter.delete('/:id', auth_middleware_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        const postId = req.params.id;
